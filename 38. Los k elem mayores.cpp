...
/* En primer lugar, anadimos los pirmeros k elementos al conjunto.
A partir de ahi, como ya superamos este numero de elementos, insertamos el nuevo elemento y borramos el minimo de los k + 1 elementos 
actuales del conjunto, de tal modo que nos quedamos con los k elementos mayores y asi nos aseguramos que el coste en espacio es O(k).
El coste en tiempo es lineal segun el numero de elementos maximos que llega a haber en el arbol, es decir, k + 1. O(k)
en el caso peor, ya que tanto insert como erase_min son lineales en el caso peor.
Mientras que si el arbol es equilibrado es coste en tiempo seria logaritmico O(log k)
*/
template <class T>
