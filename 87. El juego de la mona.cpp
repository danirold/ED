...
bool resuelveCaso() {
    int num_jugs; int cartas_rest;
    cin >> num_jugs >> cartas_rest;

    if (!std::cin)
        return false;

    vector<SetTree<int>> v(num_jugs);
    ...
    int carta;
    for (int i = 0; i < cartas_rest; ++i) {
        cin >> carta;
        if (v[i % num_jugs].contains(carta)) {
            v[i % num_jugs].erase(carta);
        }
        else {
            v[i % num_jugs].insert(carta);
        }
    }
