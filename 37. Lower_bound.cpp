...
private:
    ...
    std::pair<bool, T> lower_bound_aux(Node* node, T const& elem) const;
};
...
template <typename T>
std::pair<bool, T> SetTree<T>::lower_bound_aux(Node* node, T const& elem) const {
    if (node == nullptr) {
        return { false, -1 };
    }
    else if (elem < node->elem) {
        auto [encontrado, min] = lower_bound_aux(node->left, elem);
        if (encontrado) {
            return { encontrado, min };
        }
        else return { true, node->elem };
    }
    else if (elem > node->elem) {
        auto [encontrado, min] = lower_bound_aux(node->right, elem);
        if (encontrado) {
            return { encontrado, min };
        }
...
std::pair<bool, T> SetTree<T>::lower_bound(T const& elem) const {
    return lower_bound_aux(root_node, elem);
}
