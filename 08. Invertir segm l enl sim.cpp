...
void ListLinkedSingle::reverse_segment(int index, int length) {
    Node* primero = nth_node(index);
    Node* anterior_segmento = head;
    if (index > 0) {
        anterior_segmento = nth_node(index - 1);
    }
    Node* current = nth_node(index)->next;
    Node* previous = nth_node(index);
    int i = 0;
    while (i < length - 1) {
    ...
    primero->next = current;
    if (index != 0) {
        anterior_segmento->next = previous;
    }
    else head = previous;
}
