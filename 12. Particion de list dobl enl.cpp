...
void ListLinkedDouble::detach(Node* node) {   //O(1) por ser asignaciones
    node->next->prev = node->prev;
    node->prev->next = node->next;
}

void ListLinkedDouble::attach(Node* node, Node* position) {   //O(1) por ser asignaciones
...
void ListLinkedDouble::partition(int pivot) {   //O(num_elems), siendo num_elems el numero de elementos
    Node* current = head->next;
    Node* first_higher = nullptr;
    while (current != head) {
        Node* aux = current->next;
        if (current->value <= pivot) {
            if (first_higher != nullptr) {
                detach(current);
                attach(current, first_higher);
            }
        }
        else {
            if (first_higher == nullptr) {
                first_higher = current;
            }
        }
        current = aux;
    }
