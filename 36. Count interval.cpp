...
private:
    ...
    int count_interval_aux(Node* node, T const& lower, T const& upper) const;
};
...
template <typename T>
int SetTree<T>::count_interval_aux(Node* node, T const& lower, T const& upper) const {
    if (node == nullptr) {
        return 0;
    }
    else if (node->elem < lower) {
        return count_interval_aux(node->right, lower, upper);
    }
    else if (node->elem > upper) {
        return count_interval_aux(node->left, lower, upper);
    }
    else if (node->elem == upper) {
        return 1 + count_interval_aux(node->left, lower, upper);
    }
    else if (node->elem == lower) {
        return 1 + count_interval_aux(node->right, lower, upper);
    }
    else {
        return 1 + count_interval_aux(node->left, lower, upper) + count_interval_aux(node->right, lower, upper);
    }
}
