...
int num_aventajados(const BinTree<int>& arbol) {
    int sol = 0;
    int max_act = -1;
    int max_prev = -1;
    bool aventajado = false;
    std::queue<BinTree<int> > pending;
    if (!arbol.empty()) {
        pending.push(arbol);
        max_prev = pending.front().root();
    }
    ...
    while (!pending.empty()) {
        ...
        if (!current.left().empty()) {
            pending.push(current.left());
            if (max_prev < current.left().root()) {
                aventajado = true;
            }
            if (current.left().root() > max_act) max_act = current.left().root();
            siguiente++;
        }
        if (!current.right().empty()) {
            pending.push(current.right());
            if (max_prev < current.right().root()) {
                aventajado = true;
            }
            if (current.right().root() > max_act) max_act = current.right().root();
            siguiente++;
        }
        if (actual == 0) {
            if (aventajado) {
